...
class ChatMCPServer(BaseMCPServer):
    ...
    def __init__(self):
        super().__init__(name="chat_history", description="Manage chat history for global and per-document conversations")
    ...
    def _register_tools(self) -> None:
        self.register_tool(MCPToolDefinition(
            ...
            parameters={
                ...
                "properties": {
                    ...
                    "document_id": {"type": "string", "description": "Optional document ID for per-document chat"},
        ...
        self.register_tool(MCPToolDefinition(
            ...
            parameters={
                ...
                "properties": {
                    ...
                    "document_id": {"type": "string", "description": "Optional document ID for per-document chat"},
                    "tool_calls": {"type": "array", "description": "Optional list of tool calls made"}
                },
        ...
        self.register_tool(MCPToolDefinition(
            ...
            parameters={"type": "object", "properties": {"limit": {"type": "integer", "description": "Maximum sessions to retrieve (default: 10)"}}},
            required_params=[]
        ))
    ...
    async def execute_tool(self, tool_name: str, args: Dict[str, Any]) -> MCPToolResult:
        ...
        try:
            if tool_name == "get_chat_history":
                return await self._get_chat_history(args["session_id"], args.get("document_id"), args.get("limit", 50))
            elif tool_name == "save_message":
                return await self._save_message(args["session_id"], args["role"], args["content"], args.get("document_id"), args.get("tool_calls"))
            elif tool_name == "get_recent_sessions":
                return await self._get_recent_sessions(args.get("limit", 10))
            elif tool_name == "delete_session":
                return await self._delete_session(args["session_id"], args.get("document_id"))
            else:
    ...
    async def _get_chat_history(self, session_id: str, document_id: str | None = None, limit: int = 50) -> MCPToolResult:
        if document_id:
            messages = await ChatRepository.get_document_session(session_id, document_id, limit)
        else:
            messages = await ChatRepository.get_session(session_id, limit)
    ...
    async def _save_message(self, session_id: str, role: str, content: str, document_id: str | None = None, tool_calls: list | None = None) -> MCPToolResult:
        message = ChatMessage(session_id=session_id, document_id=document_id, role=role, content=content)
        message_id = await ChatRepository.create(message)
        return MCPToolResult(success=True, data={"message_id": message_id, "session_id": session_id})

    async def _get_recent_sessions(self, limit: int = 10) -> MCPToolResult:
        sessions = await ChatRepository.get_recent_sessions(limit)
        return MCPToolResult(success=True, data={"sessions": sessions, "count": len(sessions)})

    async def _delete_session(self, session_id: str, document_id: str | None = None) -> MCPToolResult:
        deleted_count = await ChatRepository.delete_session(session_id)
        return MCPToolResult(success=True, data={"deleted_count": deleted_count, "session_id": session_id})

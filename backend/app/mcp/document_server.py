...
class DocumentMCPServer(BaseMCPServer):
    ...
    def __init__(self):
        super().__init__(name="document_listing", description="List and manage invoice documents")
    ...
    def _register_tools(self) -> None:
        ...
        self.register_tool(MCPToolDefinition(
            ...
            parameters={"type": "object", "properties": {"query": {"type": "string", "description": "Search query to match against filenames"}}, "required": ["query"]},
        ...
        self.register_tool(MCPToolDefinition(
            ...
            parameters={"type": "object", "properties": {"document_id": {"type": "string", "description": "The document ID to delete"}}, "required": ["document_id"]},
            required_params=["document_id"]
        ))
    ...
    async def execute_tool(self, tool_name: str, args: Dict[str, Any]) -> MCPToolResult:
        ...
        try:
            if tool_name == "list_documents":
                return await self._list_documents(args.get("limit", 50))
    ...
    async def _list_documents(self, limit: int = 50) -> MCPToolResult:
        documents = await DocumentRepository.get_all(limit)
                ...
                "documents": [
                    {
                        ...
                        "metadata": {"vendor": doc.metadata.vendor, "invoice_number": doc.metadata.invoice_number, "total": doc.metadata.total, "currency": doc.metadata.currency}
                    }
    ...
    async def _search_documents(self, query: str) -> MCPToolResult:
        ...
        return MCPToolResult(
            ...
            data={
                "query": query,
                "results": [{"id": doc.id, "filename": doc.filename, "validation_status": doc.validation_status} for doc in documents],
                "count": len(documents)
            }
        )
    ...
    async def _delete_document(self, document_id: str) -> MCPToolResult:
        ...
        return MCPToolResult(success=True, data={"document_id": document_id, "deleted": True})

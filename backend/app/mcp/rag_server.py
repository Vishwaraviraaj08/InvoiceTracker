...
class RAGMCPServer(BaseMCPServer):
    ...
    def __init__(self):
        super().__init__(name="rag_query", description="Query invoice documents using Retrieval-Augmented Generation")
        self.rag_pipeline = get_rag_pipeline()

    def _register_tools(self) -> None:
        self.register_tool(MCPToolDefinition(
            name="query_document",
            description="Query a specific invoice document with a natural language question.",
            parameters={
                "type": "object",
                "properties": {
                    "document_id": {"type": "string", "description": "The ID of the document to query"},
                    "question": {"type": "string", "description": "The question to ask about the document"},
        ...
        self.register_tool(MCPToolDefinition(
            ...
            parameters={
                ...
                "properties": {
                    ...
                    "max_chunks": {"type": "integer", "description": "Maximum chunks to include (default: 10)"}
                },
        ...
        self.register_tool(MCPToolDefinition(
            ...
            parameters={"type": "object", "properties": {"document_id": {"type": "string", "description": "The ID of the document to index"}}, "required": ["document_id"]},
            required_params=["document_id"]
        ))
    ...
    async def execute_tool(self, tool_name: str, args: Dict[str, Any]) -> MCPToolResult:
        ...
        try:
            if tool_name == "query_document":
                return await self._query_document(args["document_id"], args["question"], args.get("top_k", 3))
            elif tool_name == "get_document_context":
                return await self._get_document_context(args["document_id"], args.get("max_chunks", 10))
    ...
    async def _query_document(self, document_id: str, question: str, top_k: int = 3) -> MCPToolResult:
        ...
        return MCPToolResult(
            success=True,
            data={"answer": result["answer"], "sources": result.get("sources", []), "chunks_used": result.get("chunks_used", 0)},
            metadata={"model_used": result.get("model_used")}
        )

    async def _get_document_context(self, document_id: str, max_chunks: int = 10) -> MCPToolResult:
        document = await DocumentRepository.get_by_id(document_id)
        if not document:
            return MCPToolResult(success=False, error="Document not found")
        ...
        return MCPToolResult(
            success=True,
            data={"document_id": document_id, "filename": document.filename, "context": context, "raw_text_length": len(document.raw_text)}
        )
    ...
    async def _index_document(self, document_id: str) -> MCPToolResult:
        ...
        chunks_created = await self.rag_pipeline.index_document(document_id, document.raw_text)

        return MCPToolResult(success=True, data={"document_id": document_id, "chunks_created": chunks_created, "indexed": True})

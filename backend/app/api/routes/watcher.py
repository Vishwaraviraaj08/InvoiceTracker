...
async def start_watcher(config: WatcherConfig):
    ...
    if not success:
        raise HTTPException(status_code=400, detail=f"Failed to start watching folder: {config.folder_path}")

    return {"success": True, "message": f"Started watching {config.folder_path}", "status": watcher.get_status()}
...
async def stop_watcher():
    ...
    return {"success": True, "message": "Folder watcher stopped", "status": watcher.get_status()}
...
async def get_processed_files():
    ...
    from app.services.folder_watcher import get_folder_watcher
    watcher = get_folder_watcher()
    return {"processed_files": watcher.processed_files, "total_count": len(watcher.processed_files)}
